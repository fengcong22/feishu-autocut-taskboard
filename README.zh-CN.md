...
请运行 `npm link`，以便在 shell 路径中使用 `taskctl`。设置 `CODEX_TASKBOARD_URL`，可让 CLI 指向另一个本机回环服务。云端部署通过回环 companion 使用 `taskctl cloud login` 配置。
...
当前 ZIP 获取方式只启用“手动选择”：在任务详情中选择 Auto-Cut 生成的完整 ZIP。`watch_directory` 和 `driver_report` 仍是预留配置值，目录采集器和主动上报接入前不会在界面中开放。
